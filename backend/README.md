...
- `session.py` manages one browser connection at a time. It buffers mic audio, detects end-of-speech, triggers the pipeline, tracks conversation state, and sends JSON state updates plus synthesized audio back to the client.
- `pipeline.py` contains the heavy AI orchestration layer. It loads config and models, runs speech-to-text, generates LLM replies, synthesizes TTS audio, and optionally integrates MCP tools.
